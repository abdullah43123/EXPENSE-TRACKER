...
export const GetExpenseData = async ({ User }) => {
    try {
        const data = await axios.get(`expense-tracker-backend-production-f9ad.up.railway.app/auth/getIdExpense/${User}`);
...
export const InsertIncomeData = async ({ formData }) => {
    try {
        const data = await axios.post('expense-tracker-backend-production-f9ad.up.railway.app/auth/income', formData);
...
export const GetIncomeData = async ({ User }) => {
    try {
        ...
        const data = await axios.get(`expense-tracker-backend-production-f9ad.up.railway.app/auth/getIdIncome/${User}`);
        console.log(data);
        return data
    } catch (error) {
        console.log(error);
